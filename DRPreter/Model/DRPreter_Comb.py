...
class DRPreter(nn.Module):
    ...
    def padding(self, x, mask):
        ...
        x_pad_1 = x.new_full((x.size(0), (mask[-1][0]+1) * self.max_gene, x.size(2)), 0) ### Added
        x_pad_2 = x.new_full((x.size(0), (mask[-1][1]+1) * self.max_gene, x.size(2)), 0) ### Added
        x_pad = torch.stack([x_pad_1, x_pad_2])
        ...
        # print(x_pad_1.shape, cum_num_nodes[0,:].reshape(-1).shape, x.size(0),mask[-1][1]+1)
        index_1 = (index - cum_num_nodes[0,:][mask[:,0]]) + (mask[:,0] * self.max_gene)    ### Added
        index_2 = (index - cum_num_nodes[0,:][mask[:,1]]) + (mask[:,1] * self.max_gene)    ### Added
        x_pad_1[:, index_1.long(), :] = x
        x_pad_2[:, index_2.long(), :] = x
        # x_pad = x_pad.view(x.size(0), mask[-1].item() + 1, -1)                             # x_pad.shape: torch.Size([128, 34, 2808])
        x_pad_1 = x_pad_1.view(x.size(0), mask[-1][0] + 1, -1) ### Added
        x_pad_2 = x_pad_2.view(x.size(0), mask[-1][1] + 1, -1) ### Added
        x_pad = torch.stack([x_pad_1, x_pad_2])
    ...
    def aggregate(self, x_cell, x_drug, trans=False):
        ...
        if trans:
            ...
            # -----
            x_1 = torch.cat([x_cell[0], x_drug], 1)
            x_2 = torch.cat([x_cell[1], x_drug], 1)
            x = torch.stack([x_1,x_2]).mean(dim=0)
    ...
    def forward(self, drug, cell):
        ...
        mask = cell.x_mask.reshape(-1,2)[cell.batch==0].to(torch.long)
    ...
    def _embedding(self, drug, cell):
        ...
        x_cell = self.CellEncoder(cell)
        # mask = cell.x_mask[cell.batch==0].to(torch.long)
        mask = cell.x_mask.reshape(-1,2).to(torch.long)
        x_cell = self.cell_emb(self.padding(x_cell, mask)) if self.trans else self.cell_emb(x_cell)
