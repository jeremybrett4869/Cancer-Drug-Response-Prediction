...
def main():
    ...
    rpath = './'
    result_path = rpath + 'Result/Result_5/' ### Adjust folder name
    
    print(f'seed: {args.seed}')
    set_random_seed(args.seed)
    
    edge_type = 'PPI_'+str(args.string_edge) if args.edge=='STRING' else args.edge
    edge_index = np.load(rpath+f'Data/Cell/edge_index_{edge_type}_{args.dataset}_seg_comb.npy') ### Adjust file name
    ...
    cell_dict = np.load(rpath+f'Data/Cell/cell_feature_std_{args.dataset}_seg_comb.npy', allow_pickle=True).item() 
...
# -----------------------------------------------------------------
    ...
    if args.mode == 'train':
        ...
        state_dict_name = f'{rpath}weights/weights_5/weight_sim_seed{args.seed}.pth' if args.sim==True else f'{rpath}weights/weights_5/weight_seed{args.seed}.pth'
