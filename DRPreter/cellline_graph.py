...
def save_cell_graph(gene_path, save_path, type):
    if os.path.exists(os.path.join(save_path, 'cell_feature_std_{}_seg_comb.npy'.format(type))): ### Adjust file name
        print('already exists!')
    else:
        # os.makedirs(save_path)
        ### exp = pd.read_csv(os.path.join(gene_path, 'CCLE_2369_EXP.csv'), index_col=0)
        exp = pd.read_csv(os.path.join(gene_path, 'CCLE_seg_cnv_2510.csv'), index_col=0)
        mut = pd.read_csv(os.path.join(gene_path, 'CCLE_mutation_2510.csv'), index_col=0)
        # exp = pd.read_csv(os.path.join(gene_path, 'CCLE_cnv_2510.csv'), index_col=0)
        index = exp.index
        columns = exp.columns
        ...
        mut = scaler.fit_transform(mut) ### Added

        imp_mean = SimpleImputer()
        exp = imp_mean.fit_transform(exp)
        mut = imp_mean.fit_transform(mut) ### Added

        ### Add mut
        mut = pd.DataFrame(mut, index=index, columns=columns) # mut
        exp = pd.DataFrame(exp, index=index, columns=columns) # cnv
        ...
        for i in tqdm((cell_names)):
            ...
            if type == 'joint':
                ...
                cell_dict[i] = Data(x=torch.tensor([exp.loc[i], mut.loc[i]], dtype=torch.float).T) # switch to combined data
            ...
            # disjoint graph (with pathway)
                ...
                for p, pw in enumerate(list(kegg)):
                    ...
                    for gene in kegg[pw]:
                        if gene in genes:
                            gene_list_2[pw].append(gene)
                            x_mask_2.append(p)
                    x_2.append(mut.loc[i, gene_list_2[pw]])
                x_2 = pd.concat(x_2)
                ### Combined
                # cell_dict[i] = Data(x=torch.tensor([x], dtype=torch.float).T, x_mask=torch.tensor(x_mask, dtype=torch.int))
                cell_dict[i] = Data(x=torch.tensor([x_2,x], dtype=torch.float).T, x_mask=torch.tensor([x_mask_2,x_mask], dtype=torch.int))

        print(cell_dict)
        np.save(os.path.join(save_path, 'cell_feature_std_{}_seg_comb.npy').format(type), cell_dict) ### Adjust file name
        print("finish saving cell data!")
        return gene_list


def get_STRING_edges(gene_path, ppi_threshold, type, gene_list):
    save_path = os.path.join(gene_path, 'edge_index_{}_{}_seg_comb.npy'.format(ppi_threshold, type)) ### Adjust file name
    if not os.path.exists(save_path):
        # gene_list
        ppi = pd.read_csv(os.path.join(gene_path, 'CCLE_2510_{}.csv'.format(ppi_threshold)), index_col=0)
        ...
        np.save(
            os.path.join(rpath + 'Data/Cell/', 'edge_index_{}_{}_seg_comb.npy'.format(ppi_threshold, type)), 
            edge_index) ### Adjust file name
    else:
...
if __name__ == '__main__':
    ...
    with open(gene_path+'/34pathway_score990_isolated.pkl', 'rb') as file: # isolated
