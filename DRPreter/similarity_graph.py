...
from scipy.stats import pearsonr, spearmanr # spearman
import argparse
...
with open(dict_dir + "drug_name_cell_id_auc", 'rb') as f:
    drug_name_cell_id_ic50 = pickle.load(f)
# with open(dict_dir + "drug_idx_cell_idx_ic50", 'rb') as f:
with open(dict_dir + "drug_idx_cell_idx_auc", 'rb') as f:
...
cell_feature_normalized = np.load(rpath+f'Data/Cell/cell_feature_std_disjoint_seg_cnv.npy', allow_pickle=True).item()
...
def computing_sim_matrix():
    ...
    for i in range(len(cell_id2idx_dict)):
        for j in range(len(cell_id2idx_dict)):
            if i != j:
                # cell_sim_matrix[i, j], _ = pearsonr(cell_feature_normalized[i], cell_feature_normalized[j]) ??? KeyError: 0
                cell_sim_matrix[i, j], _ = \
                pearsonr(np.array(cell_feature_normalized[cell_idx2id_dict[i]], dtype=object)[0][1].reshape(-1),\
                         np.array(cell_feature_normalized[cell_idx2id_dict[j]], dtype=object)[0][1].reshape(-1))
...
def computing_knn(k):
    ...
    with open(dir + "edge/drug_cell_edges_{}_knn_seg_cnv".format(k), 'wb') as f:
